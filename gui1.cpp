...
std::function<void(float)> f = [](float) {
	std::cout << " slicer test" << std::endl;
	return 1.11;
};
...
public:
	ExampleApplication() : nanogui::Screen(Eigen::Vector2i(1000, 800), "Minibot", false) {
		using namespace nanogui;
		Window *window1 = new Window(this, "Gridlayout");
		GridLayout *grid = new GridLayout();
		grid->setSpacing(0.5);
		grid->setResolution(1);
		window1->setPosition(Vector2i(400, 400));
		window1->setLayout(grid);
		Graph *graph;

		graph = new Graph(window1,"Plot");
		
		graph->setBackgroundColor({ 255, 255, 0, 255 });
		graph->setSize({ 400, 400 });
		Widget *tools1 = new Widget(window1);
		tools1->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Fill, 0, 5));
		Button *button = new Button(tools1, "Change Color");
		button->setCallback([this]() { std::cout << "call graph" << std::endl; });


		Window *window = new Window(this, "Cube");
		window->setPosition(Vector2i(15, 15));
		window->setLayout(new GroupLayout());
		
		mCanvas = new MyGLCanvas(window);
		
		mCanvas->setBackgroundColor({ 255, 255, 255, 255 });
		mCanvas->setSize({ 400, 400 });

		Widget *tools = new Widget(window);
		tools->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Fill, 0, 5));
		...
		Button *b2 = new Button(tools, "Print");
		b2->setCallback([this]() {std::cout << "press button" << std::endl; });
		...
		//slider->finalCallback([this](float) {});
		//slider->setCallback([this](float) {});
		float data;
		slider->setValue(0.1);
		std::pair<float, float> range(-1.0f, 1.0f);
		slider->setRange(range);
		
		std::pair<float, float> range1(data,-1.0f);
		slider->setHighlightedRange(range1);
		
		
		slider->setFinalCallback([&data](float value) {
			data = value * 1.0;
